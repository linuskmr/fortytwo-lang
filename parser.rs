...
//!
use ast::AST;
use ast::error::{ParsingError, ParsingErrorKind};
use lexer::{Lexer, PositionRange, PositionRangeContainer, Token, TokenType, UnknownSymbolError};

/// A parser of tokens generated by its [Lexer].
struct Parser< R: Iterator<Item=String>> {
...
impl<R: Iterator<Item=String>> Parser<R> {
    ...
    fn get_next_token(&mut self) -> &Option<Result<Token, UnknownSymbolError>> {
    ...
    fn parse_function_prototype(&mut self) -> Result<ast::FunctionPrototype, ParsingError> {
        ...
        match self.get_next_token() {
            Some(Token { data: TokenType::OpeningParentheses, .. }) => (), // Ok
            _ => return Err(ParsingError {
                kind: ParsingErrorKind::ExpectedSymbol,
    ...
    fn parse_parentheses(&mut self) -> ParseResult {
        ...
        if let Some(Token { data: TokenType::ClosingParentheses, .. }) = self
            .current_token {
