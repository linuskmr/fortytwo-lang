...
pub fn parse_function_definition(tokens: &mut Peekable<impl Iterator<Item = Token>>) -> Result<ast::statement::FunctionDefinition> {
...
fn parse_function_prototype(tokens: &mut Peekable<impl Iterator<Item = Token>>) -> Result<ast::statement::FunctionPrototype> {
...
fn parse_function_argument_list(tokens: &mut Peekable<impl Iterator<Item = Token>>) -> Result<Vec<ast::statement::FunctionArgument>> {
...
fn parse_function_prototype_return_type(tokens: &mut Peekable<impl Iterator<Item = Token>>) -> Result<Option<PositionContainer<ast::statement::DataType>>> {
	match tokens.peek() {
		// No return type specified
		Some(Token { inner: TokenKind::OpeningCurlyBraces, .. }) => {
			Ok(None)
		},
...
pub(crate) fn parse_function_call(tokens: &mut Peekable<impl Iterator<Item = Token>>, identifier: PositionContainer<String>) -> Result<ast::expression::FunctionCall> {
