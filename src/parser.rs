...
impl<TokenIter: Iterator<Item = Token>> Parser<TokenIter> {
    ...
    pub fn new(tokens: TokenIter, named_source: Arc<NamedSource>) -> Self {
        Self { tokens: tokens.peekable(), named_source }
    }
    ...
    fn parse_function_prototype(&mut self) -> miette::Result<FunctionPrototype> {
        ...
        let name = match self.tokens.next() {
            ...
            other => return Err(error::ExpectedIdentifier {
                src: self.named_source.clone(),
                err_span: other.map(|token| token.position).unwrap_or(SourceSpan::new(0.into(), 0.into())) // TODO: Better position
            }.into())
        };
        ...
        match self.tokens.peek() {
            ...
            other => {
                return Err(error::ExpectedOpeningRoundParentheses {
                    src: self.named_source.clone(),
                    err_span: other.map(|token| token.position.clone()).unwrap_or(SourceSpan::new(0.into(), 0.into())) // TODO: Better position
                }.into())
            }
        }
        ...
        match self.tokens.next() {
            Some(Token { data: TokenKind::Identifier(type_str), position}) if type_str == "ptr" => {
                ...
                Ok(PositionContainer {
                    ...
                    position: SourceSpan::new(position.offset().into(), type_to_point_to.position.len().into())
                })
            }
    ...
    fn consume_closing_parentheses(&mut self) -> miette::Result<()> {
        match self.tokens.next() {
            ...
            other => return Err(error::ExpectedClosingRoundParentheses {
                src: self.named_source.clone(),
                err_span: other.map(|token| token.position).unwrap_or(SourceSpan::new(0.into(), 0.into())) // TODO: Better position
            }.into()),
        }
    ...
    fn parse_for_expression(&mut self) -> miette::Result<ForLoop> {
        // Consume for
        assert_eq!(self.tokens.next().map(|token| token.data), Some(TokenKind::For));
        // Read setup
        let setup = self.parse_binary_expression()?;
        // Check and consume semicolon `;`
        match self.tokens.next() {
            Some(Token { data: TokenKind::Semicolon, .. }) => (),
            other => return Err(error::ExpectedSemicolon {
                src: self.named_source.clone(),
                err_span: other.map(|token| token.position).unwrap_or(SourceSpan::new(0.into(), 0.into())) // TODO: Better position
            }.into())
        };
        ...
        match self.tokens.next() {
            ...
            other => return Err(error::ExpectedOpeningCurlyParentheses {
                src: self.named_source.clone(),
                err_span: other.map(|token| token.position).unwrap_or(SourceSpan::new(0.into(), 0.into())), // TODO: Better position
        ...
        match self.tokens.next() {
            ...
            other => return Err(error::ExpectedClosingCurlyParentheses {
                src: self.named_source.clone(),
                err_span: other.map(|token| token.position).unwrap_or(SourceSpan::new(0.into(), 0.into())) // TODO: Better position
            }.into())
        };
    ...
    fn parse_if_expression(&mut self) -> miette::Result<IfElseExpression> {
        ...
        match self.tokens.next() {
            ...
            other => return Err(error::ExpectedElse {
                src: self.named_source.clone(),
                err_span: other.map(|token| token.position).unwrap_or(SourceSpan::new(0.into(), 0.into())) // TODO: Better position
            }.into())
        };
