...
use ast::{AST, FunctionPrototype};
use lexer::{Lexer, PositionRange, PositionRangeContainer, Token, token::Precedence, token::SpecialCharacter, TokenType};
...
struct Parser<R: io::Read> {
    ...
    previous_position: Option<PositionRange>,
}
...
impl<R: io::Read> Parser<R> {
    ...
    /// than `+`.
    fn parse_binary_operation_rhs(&mut self, min_operator: Option<SpecialCharacter>,
                                  lhs: Box<AST>) -> ParseResult {
        let mut lhs = lhs;
        loop {
            // Read the operator
            let operator = match &self.current_token {
                Some(Token { data: TokenType::Other(op), position }) => {
                    PositionRangeContainer { data: op.clone(), position: position.clone() }
                }
                // Expression ended here
                _ => return Ok(lhs),
            };
            if operator_banned(&operator.data, &min_operator) {
                return Ok(lhs);
            }
            ...
            let next_operator = self.current_token.as_ref()
                .and_then(|tok| match tok {
                    Token { data: TokenType::Other(sc), .. } => Some(sc.clone()),
                    _ => None,
                });

            // Inspect next binary operator
            if operator_banned(&operator.data, &next_operator) {
    ...
    fn parse_function_prototype(&mut self) -> Result<ast::FunctionPrototype, ParsingError> {
        let func_name = match &self.current_token {
            ...
            _ => return Err(ParsingError {
                ...
                message: format!("Expected identifier for function prototype"),
                position: self.current_position(),
            }),
        };

        match self.get_next_token() {
            Some(Token { data: TokenType::Other(SpecialCharacter('(')), .. }) => (), // Ok
            _ => return Err(ParsingError {
                kind: ParsingErrorKind::ExpectedSymbol,
                message: format!("Expected `(` in function prototype"),
                position: self.current_position(),
            })
        }
            ...
            match self.get_next_token() {
                Some(Token { data: TokenType::Identifier(arg_name), position: position }) => {
                    arg_names.push(PositionRangeContainer {
                        data: arg_name.clone(), position:  position.clone()
                    });
                }
                _ => break,
            }
        ...
        match &self.current_token {
            Some(Token { data: TokenType::Other(SpecialCharacter(')')), .. }) => (),
            _ => return Err(ParsingError {
                kind: ParsingErrorKind::ExpectedSymbol,
                message: format!("Expected `)` in function prototype"),
                position: self.current_position(),
    ...
    fn parse_parentheses(&mut self) -> ParseResult {
        ...
        if let Some(Token { data: TokenType::Other(SpecialCharacter(')')), .. }) = self
            .current_token {
        ...
        loop {
            ...
            match self.current_token {
                ...
                _ => {
                    ...
                    return Err(ParsingError {
                        kind: ParsingErrorKind::ExpectedSymbol,
                        message: format!("Expected `)` or `,` in argument list"),
                        position: self.current_position(),
                    });
                }
            };
    ...
    fn parse_primary_expression(&mut self) -> ParseResult {
        match self.current_token.clone() {
            ...
            _ => Err(ParsingError {
                ...
                message: format!("Expected (primary) expression"),
                position: self.current_position(),
            }),
        }
    }

    fn current_position(&self) -> Option<PositionRange> {
        self.current_token.as_ref()
            .and_then(|tok| Some(tok.position.clone()))
            .or(self.previous_position.clone())
    }
}

fn operator_banned(operator: &SpecialCharacter, min_operator: &Option<SpecialCharacter>) -> bool {
